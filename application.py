        ...
        if 'category_select' in request.form:
            try:
                cat = session.query(Category).\
                    filter_by(name=request.form['category_select']).one()
            except:
                flash('Error fulfilling request.')
                return redirect(url_for('index'))
            ...
            try:
                ...
                try:
                    category = session.query(Category).\
                        filter_by(name=new_cat_name).one()
                except:
                    flash('Error fulfilling request.')
                    return redirect(url_for('index'))
...
def gen_item_slug(base):
    """
    gen_item_slug - generates a slug out of the provided base string and 5
...
def login():
    ...
    # handle POST requests to this endpoint
        ...
        # verify that this request came from a user attempting to log in via
        # the login page and not from some bot hitting this endpoint randomly
...
def categories_json():
    """
    categories_json - returns a JSON representation of all the categories in
    the DB.
    """
    category_list = session.query(Category).order_by(asc(Category.name))
